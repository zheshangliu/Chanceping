/**
 * GLM (Z.AI) LLM 适配器（glm_adapter）
 *
 * 来源：Task 020 第 4.2 节。
 *
 * 实现 LLMAdapter 接口，对接 Z.AI (智谱 GLM) API（OpenAI 兼容模式）。
 * 支持 Mock 降级：无 ZAI_API_KEY 时自动进入 Mock 模式，不抛错。
 *
 * 三条路径：
 *   1. Mock 模式（无 apiKey）：根据 user 消息关键词匹配返回预设 JSON/文本
 *   2. 真实模式（有 apiKey）：fetch 调用 Z.AI API，OpenAI 兼容模式
 *   3. JSON 修复：response_format="json" 时复用 T4 parseJsonWithRepair
 *
 * Mock 预设 JSON 结构与 QwenAdapter / DeepSeekAdapter 完全一致，
 * ConversationManager 无感切换。
 *
 * GLM-4.7-Flash 完全免费，30B/3B MoE，支持 Tool Calling，可本地部署。
 * 商业版策略中作为批量初筛主力（零成本）。
 *
...
export interface GlmConfig {
  /** Z.AI API Key（从 env ZAI_API_KEY 读取） */
  apiKey: string;
  /** 模型名，默认 "glm-4.7-flash" */
  model: string;
  /** API 基础地址，默认 "https://api.z.ai/api/paas/v4" */
  baseUrl: string;
  /** 最大 token 数，默认 4096 */
  maxTokens?: number;
...
// ============================================================
// Mock 预设响应（与 QwenAdapter / DeepSeekAdapter 完全一致）
// ============================================================

/**
 * 机会提取预设 JSON（含"机会"/"评分"/"提取"关键词时返回）。
 *
 * 结构与 QwenAdapter / DeepSeekAdapter / MockLLMAdapter 的机会提取响应保持一致，
 ...
 *
 * 兼容 QwenAdapter / DeepSeekAdapter / MockLLMAdapter 行为。
 */
...
/** 通用预设文本（无关键词匹配时返回） */
const MOCK_GENERIC_TEXT = "Mock 模式：未匹配到特定关键词，返回通用文本响应。请设置 ZAI_API_KEY 以启用真实 GLM 调用。";
 ...
 *
 * 工作流程：
 *   1. 构造器读取 config 或 env ZAI_API_KEY，无 key 时进入 Mock 模式
 *   2. Mock 模式：根据 user 消息关键词匹配返回预设 JSON/文本
 *   3. 真实模式：fetch 调用 Z.AI API（OpenAI 兼容模式）
...
export class GlmAdapter implements LLMAdapter {
  ...
  constructor(config?: Partial<GlmConfig>) {
    const envKey = typeof process !== "undefined" ? process.env?.ZAI_API_KEY ?? "" : "";
   ...
   *
   * 匹配优先级（与 QwenAdapter / DeepSeekAdapter 一致）：
   *   1. 含"机会"/"评分"/"提取" → 机会提取预设
   *   2. 含"需求"/"确认"/"理解" → 需求理解预设
   *   3. 其他 → 通用预设
   */
  ...
  /**
   * 真实模式调用：fetch Z.AI API（OpenAI 兼容模式）。
   *
   * 请求：
  ...
  private async chatReal(request: LLMRequest): Promise<LLMResponse> {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `GLM API error: status=${response.status}, body=${errorText.slice(0, 200)}`,
          );
        }
    ...
    throw new Error(
      `GLM API call failed after retry: ${lastError?.message ?? "unknown error"}`,
    );
  }
